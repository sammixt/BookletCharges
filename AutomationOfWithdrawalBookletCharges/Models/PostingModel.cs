...
namespace AutomationOfWithdrawalBookletCharges.Models
{
    public class PostingModel
    {
        ...
        public static List<PostingViewModel> creditAccount(string transid,string payRefId)
        {
            List<PostingViewModel> creditDetail = new List<PostingViewModel>();
            creditDetail.Add(new PostingViewModel {
                accountNumber = ConfigurationManager.AppSettings["CrComm"].ToString(),
                amount = 258,amountSpecified = true,branchCode = "",
                debitCreditIndicator = "C",glCasaIndicator = "GL",
                narration = "CR Comm. On Savings W/D Booklet(Income)",
                paymentReference = payRefId,
                transactionId = transid
            });
            creditDetail.Add(new PostingViewModel
            {
                accountNumber = ConfigurationManager.AppSettings["CostOfBklt"].ToString(),
                amount = 42,
                amountSpecified = true,
                branchCode = "",
                debitCreditIndicator = "C",
                glCasaIndicator = "GL",
                narration = "CR Savings Withdrawal Booklet (cost of booklet)",
                paymentReference = payRefId,
                transactionId = transid
            });
            creditDetail.Add(new PostingViewModel
            {
                accountNumber = ConfigurationManager.AppSettings["CRVAT"].ToString(),
                amount = 15,
                amountSpecified = true,
                branchCode = "",
                debitCreditIndicator = "C",
